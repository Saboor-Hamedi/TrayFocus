...
const ChatPanel = ({ apiKey, providerId, model }) => {
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      const result = await sendMessage(providerId, allMessages, apiKey, {
        model,
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.05] shrink-0">
        ...
        {messages.length > 0 && (
          <button
            onClick={() => setMessages([])}
            className="size-6 flex items-center justify-center rounded-md text-white/20 hover:text-white/50 hover:bg-white/5 transition-colors"
            title="Clear chat"
          >
      ...
      {error && (
        <div className="px-4 py-1.5 border-t border-red-500/5 bg-red-500/[0.03] flex items-center gap-2 shrink-0">
