...
export const ChatMessage = ({ message, fontSize = 14, accentColor = '' }) => {
  ...
  if (isUser) {
    return (
      <div className="group flex flex-col items-end px-4 py-1">
        <div
          className="max-w-[75%] bg-white/[0.08] border border-white/[0.06] text-white/85
  ...
  return (
    <div className="group flex flex-col px-4 py-1">
      <div
        className={`w-full leading-relaxed ${message.error ? 'text-red-400/90' : 'text-white/80'}`}
        style={{ fontSize: fs }}
      >
        {isEmpty
          ? <TypingDots />
...
const ChatInput = ({ onSend, onStop, loading, disabled, placeholder = 'Message...' }) => {
  ...
  return (
    <div className="shrink-0 px-3 pb-3 pt-2 border-t border-white/[0.05]">
      <div className="max-w-xl mx-auto">
        <div
          className="flex flex-col rounded-xl overflow-hidden border transition-all duration-200
            bg-white/[0.04] border-white/[0.07]
            focus-within:bg-white/[0.055] focus-within:border-white/[0.14]
            focus-within:shadow-[0_0_0_1px_rgba(255,255,255,0.04)]"
        >
          {/* Textarea */}
          <textarea
            ref={textareaRef}
            ...
            rows={2}
            className="w-full bg-transparent px-3.5 pt-3 pb-2 text-[13px] text-white/90
              outline-none placeholder:text-white/20 resize-none disabled:opacity-30
          ...
          <div className="flex items-center justify-between px-3 py-1.5 border-t border-white/[0.05]">
            <span className="text-[9px] text-white/20 select-none flex items-center gap-1 leading-none">
              ...
              {loading ? (
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg
                    ...
                    text-white/50 hover:text-white/80 text-[10px] font-medium
